...
\texttt{x\_hat} contains the states of the system, being the angle of the frame (\texttt{x\_hat[0]}), the velocity of the frame (\texttt{x\_hat[1]}) and the velocity of the wheel (\texttt{x\_hat[2]}). \texttt{x\_hat} is the equivalent of vector \si{\vec{x}}. The inner product between \si{\vec{K}} and \si{\vec{x}} is calculated on line 11 in code listing \ref{codeStateSpaceControl}. On line 14 the found output torque is converted to a current, since the motor control board takes a current as reference. The \texttt{TORQUE\_2\_CURRENT} constant is found by taking the inverse of the motor torque constant.
