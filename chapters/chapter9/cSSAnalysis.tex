...
\begin{flalign}  \label{controlability}
	\si{\vec{\zeta}} = 
	\begin{bmatrix}
		\vec{B} & \vec{A}\vec{B} & \vec{A}^2\vec{B} \\
	\end{bmatrix}
	\si{=}
	\begin{bmatrix}
		0 & -149 & 175 \\
...
Another important characteristic for the design of controllers is the observability, which refers to the capability of inferring the internal states knowing the outputs of the system. In this case there exists a matrix which includes only \si{\vec{C}} and \si{\vec{A}}, which means that the observability has no relation with the inputs of the system. 
%
...
\end{flalign}
%
This observability matrix, \si{\vec{{\mathcal O}}}, also has full rank so the states can be observed.