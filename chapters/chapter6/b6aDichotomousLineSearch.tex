...
This process of reducing the range of the bracket is illustrated in \figref{dichotomousLargerAorB}, where the green and red functions are examples of how f(x) could behave given the red known points. This leaves the two possible intervals for the minimum \si{x^*}. In the case of \si{f(x_a) < f(x_b)}, in \figref{dichotomousLargerB}, the possible intervals, \si{x_{L} < x^* < x_a} and \si{x_a < x^* < x_b}, can be combined to the new bracket \si{x_{L} < x^* < x_b}, shown in blue, which is sure to contain the minimum, \si{x^*}, within \si{x_{L}} and \si{x_{U}}.\\
