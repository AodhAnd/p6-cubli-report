...
Based on the setup of the complementary filter as seen in \figref{blockDrawingComplementaryFilter} it was decided to start out by the same cut-off frequency for both low and high pass filter, since it is desired to find the angle of the frame with a gain of 1.
...
The final fit can be seen in \figref{filterSensTool} and it gives an optimal time constant \si{\tau\ =\ 0,5399\ s}. This results in a cut-off frequency for the filter equal to \si{11,63\ rad \cdot s^{-1}}.
%
\begin{figure}[H]
	\centering
