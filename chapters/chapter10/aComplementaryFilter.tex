...
Based on the setup of the complementary filter as seen in \figref{blockDrawingComplementaryFilter} the same cut-off frequency is chosen for both low and high pass filter, since it is desired to find the angle of the frame with a gain of 1. In \figref{bodeFilters} it is shown how a summation of the gain from the low and high pass filter, when both of them have the same cut-off frequency.
%
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{figures/bodeFilters}
	\caption{Magnitude Bode diagrams of the high pass and low pass filters from the complementary filter as well as the summation of both filters to show the resulting gain of the angle}
	\label{bodeFilters}
\end{figure}\vspace{-5mm}
%
