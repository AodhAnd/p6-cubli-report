...
\section{Data Fusing} \fxnote{better section name?}
The complementary filter is used to combine the measurement data from the accelerometer and gyros in the two IMUs mounted on the Cubli frame. 
...
In oder to use the complementary filter on the cubli it needs to be discretized. This is done with the bilinear transformation method where \si{s = \frac{2}{T\cdot \frac{1 - z^{-1}}{1 + z^{-1}}}}
...
Now s is replaced
\begin{flalign}
  	\eq{\theta_{F}} {\frac{1}{1 + \tau \cdot \frac{2}{T\cdot \frac{1 - z^{-1}}{1 + z^{-1}}} \cdot (accel\_\theta_{F} + \tau \cdot gyro\_\dot{\theta}_{F})}}
  	\label{discreteComplementaryFilter2}
\end{flalign}
  
\begin{flalign}
  	\eq{\Updownarrow \theta_{F}} {\frac{T \cdot (1 + z^{-1})}{ T \cdot (1 + z^{-1})+2\cdot (1 - z^{-1})\cdot \tau} \cdot (accel_{\theta_{F}} + \tau \cdot gyro_{\dot{\theta}_{F}})}
  	\label{discreteComplementaryFilter3}
\end{flalign}
  
\begin{flalign}
   	\eq{\Updownarrow \theta_{F}} {\frac{ T +  T \cdot z^{-1}}{(2\cdot \tau +  T) - (2\tau - T)\cdot z^{-1}} \cdot (accel\_\theta_{F} + \tau \cdot gyro\_\dot{\theta}_{F})}
