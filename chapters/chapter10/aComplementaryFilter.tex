...
To determine the angle of the frame with the accelerometer it is needed to get the measurements from two of the three axes. The two measurements needed are the ones in line with the frames movement direction, based on the way the IMU is mounted, as shown in \figref{accelerometer}. Based on the markings on the IMU, those are the x and y-axis directions.\\
Taking the two axis measurements shown in figure \ref{accelerometer} the angle can be found using \eqref{accelAngle} \cite{CFisher}. The offset is added because the IMU is mounted with a \si{\frac{\pi}{4}} rotation compared to the orientation of the frame.
\begin{figure}[H]
	\centering
...
\begin{flalign}
	...
	\label{accelAngle}
\end{flalign}
The offset has the value \si{\frac{\pi}{4} + 0,06 = 0,84}. The \si{0,06} comes from the fact that the frame is slightly off center. \fxnote{reference the section where it is explained the first time}
...
\end{figure}\vspace{-5mm}
%
While the Cubli setup tries to balance the frame, the IMU mounted on the top of it will move along with the frame and the spikes seen on the calculated angle data from the accelerometer data in \ref{angleAcc} are because the IMU is moving and the acceleration from this creates a disturbance on the measurement of the gravitational acceleration \cite{JWarren}.\\
