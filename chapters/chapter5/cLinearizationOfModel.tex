...
%
\begin{flalign}
	\si{(J_F+m_w \cdot {l_w}^{2}) (\vec{\ddot{\theta}}_F + \Delta \ddot{\theta}_F )} &= \si{- B_F \cdot (\vec{\dot{\theta}}_F + \Delta \dot{\theta}_F) }   \nonumber\\
	...
	\eq{(J_F+m_w \cdot {l_w}^{2}) (\vec{\ddot{\theta}}_F + \Delta \ddot{\theta}_F )}{ f( (\vec{\dot{\theta}}_F + \Delta \dot{\theta}_F), \ (\vec{\theta}_F + \Delta \theta_F), \ (\vec{\tau}_m + \Delta \tau_m),\  (\vec{\dot{\theta}}_w + \Delta \dot{\theta}_w) ) } \unit{N \cdot m}
...
The operating point is chosen as \si{\vec{\theta}_F} and \si{\vec{\theta}_w} and their derivatives being equal to 0. This corresponds to the frame being in the upright position, see \figref{cubliMechanical}. At this position all the velocities and accelerations are 0, which results in 0 torque \si{\tau_m} as well.
