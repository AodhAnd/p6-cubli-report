...
In both cases the velocity is different from 0 in the steady-state, with means that the Cubli is not in the equilibrium position defined for it. 
...
Moreover, the response of the real Cubli with the controller can also be tested (see Appendix \ref{app:sisoToolDControllerTest}). The torque asked to the motor and the position are shown in \figref{torqueTustinPre} and \figref{positionTustinPre}.
...
\begin{minipage}{0.45\linewidth}
	\begin{figure}[H]
		...
		\captionsetup{justification=centering}
		\captionof{figure}{Torque asked to the motor}
